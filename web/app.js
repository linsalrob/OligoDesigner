import { generateOligos, generateStructuredOligos, logoMatrix, parseSequenceFile, toFasta, toJson, toTsv } from "./core.js";
...
function renderLogo(sequences, options) {
  ...
  matrix.forEach((column,i)=>{const x=margin.left+i*plotWidth/matrix.length, w=Math.max(1,plotWidth/matrix.length*.9); let y=margin.top+plotHeight; const bases=["A","C","G","T"]; if(options.stackOrder==="value")bases.sort((a,b)=>column[b]-column[a]); for(const base of bases){const h=column[base]/max*plotHeight;if(h>.3){y-=h;body+=`<rect x="${x}" y="${y}" width="${w}" height="${h}" fill="${colors[base]}"/><text x="${x+w/2}" y="${y+h/2}" text-anchor="middle" dominant-baseline="central" fill="white" font-family="Arial" font-weight="700" font-size="${Math.min(w*.7,h*.7)}">${base}</text>`;}} if(matrix.length<=60)body+=`<text x="${x+w/2}" y="${margin.top+plotHeight+18}" text-anchor="middle" font-size="9">${i+1}</text>`;});
