...
function handleError(error, result = document.querySelector("#results")) {
...
document.querySelector("#logo-form").addEventListener("submit",async event=>{event.preventDefault();const result=document.querySelector("#logo-result");try{const file=fileInput.files[0];if(!file)throw new Error("Choose a FASTA or JSON file.");const data=new FormData(event.currentTarget),sequences=parseSequenceFile(await file.text(),data.get("format"),file.name),options=Object.fromEntries(data);const svg=renderLogo(sequences,options);result.className="result";result.removeAttribute("role");result.innerHTML=`<div class="result-head"><div><p class="eyebrow">Rendered locally</p><h2>${sequences.length} sequences</h2></div><div class="download-links"><button id="download-svg">Download SVG</button></div></div><div class="logo-wrap">${svg}</div>`;result.querySelector("#download-svg").addEventListener("click",()=>download(svg,"sequence-logo.svg","image/svg+xml"));result.classList.remove("hidden");result.scrollIntoView({behavior:window.matchMedia("(prefers-reduced-motion: reduce)").matches?"auto":"smooth"});}catch(error){handleError(error,result);}});
